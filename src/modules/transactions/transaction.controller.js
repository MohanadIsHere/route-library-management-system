...
import Transaction, {
  status,
} from "../../database/models/transaction.model.js";
...
export const borrowBook = async (req, res, next) => {
  try {
    ...
    if (book.availableCopies < numberOfCopies) {
      const error = new Error("No enough copies available");
...
export const returnBook = async (req, res, next) => {
  try {
    ...
    if (transaction.numberOfCopies < 0) {
...
export const getAllTransactions = async (req, res, next) => {
  try {
    if (req?.user?.role !== roles.admin) {
      const error = new Error("You have to be an admin to form this action.");
      error.statusCode = 401;
      throw error;
    }
    const transactions = (await Transaction.find().sort({ borrowDate: 1 }))
    if(!transactions) {
